...
describe('GrimoirePlugin', () => {
  ...
  describe('migration', () => {
    it('on onload with data.spellOverrides containing a real-path record, the migration runs and invokes writeBlock/persist before popupModule.register', async () => {
      const migrateModule = await import('../src/infra/migrateSpellOverrides');
      const migrateSpy = vi.spyOn(migrateModule, 'migrateSpellOverridesToFrontmatter').mockResolvedValue(undefined);

      await plugin.onload();

      expect(migrateSpy).toHaveBeenCalledOnce();
      const deps = migrateSpy.mock.calls[0][0] as any;
      expect(deps.data).toBe(plugin.data);
      expect(typeof deps.resolveFile).toBe('function');
      expect(typeof deps.writeBlock).toBe('function');
      expect(typeof deps.persist).toBe('function');
      expect(typeof deps.isSentinelPath).toBe('function');

      migrateSpy.mockRestore();
    });

    it('on onload with empty spellOverrides, the migration runs without throwing', async () => {
      const migrateModule = await import('../src/infra/migrateSpellOverrides');
      const migrateSpy = vi.spyOn(migrateModule, 'migrateSpellOverridesToFrontmatter').mockResolvedValue(undefined);
    ...
    it('migration runs before popupModule.register so first popup open reads post-migration state', async () => {
      const migrateModule = await import('../src/infra/migrateSpellOverrides');
      let migrationCalled = false;
      const migrateSpy = vi.spyOn(migrateModule, 'migrateSpellOverridesToFrontmatter').mockImplementation(async () => {
        migrationCalled = true;
      });

      // Spy on PopupModule.register to verify migration ran before it
      const PopupModuleModule = await import('../src/main/PopupModule');
      const OriginalPopupModule = PopupModuleModule.PopupModule;
      const registerSpy = vi.spyOn(OriginalPopupModule.prototype, 'register').mockImplementation(function() {
        // At this point, migration should have already been called
