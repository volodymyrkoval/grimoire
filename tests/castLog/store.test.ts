...
describe('CastLogStore', () => {
  ...
  describe('remote write option', () => {
    it('recordCasted with remote:true appends to remote path', async () => {
      const appendLine = vi.fn().mockResolvedValue(undefined);
      const getLogPathAbs = () => '/vault/.obsidian/plugins/grimoire/cast-log-local.jsonl';
      const getRemoteLogPathAbs = () => '/vault/.obsidian/plugins/grimoire/cast-log-remote.jsonl';
      const now = () => new Date('2026-01-01T00:00:00.000Z');
      ...
      expect(appendLine).toHaveBeenCalledWith(
        '/vault/.obsidian/plugins/grimoire/cast-log-remote.jsonl',
        expect.any(String)
      );
    });

    it('recordError with remote:true appends to remote path', async () => {
      const appendLine = vi.fn().mockResolvedValue(undefined);
      const getLogPathAbs = () => '/vault/.obsidian/plugins/grimoire/cast-log-local.jsonl';
      const getRemoteLogPathAbs = () => '/vault/.obsidian/plugins/grimoire/cast-log-remote.jsonl';
      const now = () => new Date('2026-01-01T00:00:00.000Z');
    ...
    it('recordCasted with remote:true but no getRemoteLogPathAbs throws', async () => {
      const appendLine = vi.fn().mockResolvedValue(undefined);
      const getLogPathAbs = () => '/vault/.obsidian/plugins/grimoire/cast-log-local.jsonl';
      const now = () => new Date('2026-01-01T00:00:00.000Z');
      ...
      await expect(
        ...
        'CastLogStore: remote write requested but getRemoteLogPathAbs is not configured'
      );
    });
  });
});
