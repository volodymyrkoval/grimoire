...
- [x] D7: integration test: write `tests/integration/options-panel-popup.spec.ts` covering the six assertions above (popup-level seam). Use the updated harness with `optionsCastAction: vi.fn()` and a pre-loaded `SpellOverrideStore` for the dot test. — M, ui-integration-tester
