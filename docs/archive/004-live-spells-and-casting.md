...
- [x] C1: add `export class Workspace { getActiveFile = vi.fn<() => any>(() => null); }` to `tests/__mocks__/obsidian.ts`. Add `workspace = new Workspace();` as a property initializer on the `App` class. Run `npm test` and `npm run test:integration` — both must remain green (no test asserts on `app.workspace` yet). — S, junior-dev
