...
- [x] E1: After `/done` and `/spec`, verify `docs/features/spell-hotkeys.md`'s paragraph beginning "The Forge dialog (both create and update modes) grows an optional **Hotkey** input that snaps every keystroke…" has been replaced (by the feature-documenter) with a description of the new button-driven capture surface, including the persisted-chip `×` erase path and the four rejection reasons. No file edits expected from this todo directly. — S, junior-dev
