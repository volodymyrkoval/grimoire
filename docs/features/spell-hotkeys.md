...
The Forge dialog (both create and update modes) grows an optional **Hotkey** field. Since `grimoire-forge-dialog-hotkey-capture` (`dev/done-030`), that field is a button-driven capture surface rather than a plain text input — see that doc for the chip / `Save` / `×` state machine and the inline commit-time validator that surfaces collisions in place. The Forge meta-spell is instructed to write `grimoire-hotkey: <value>` into the new file's frontmatter, mirroring the existing `grimoire-execute-on-note` precedent. Collisions are also resolved at popup-open as a backstop for frontmatter hand-edits: sentinels register first, then vault spells in scan order; later entries that collide or break prefix-disjointness are dropped, and a single `Notice` lists every dropped entry.
...
- **Live collision warnings in the Forge dialog** — initially deferred; subsequently delivered by `grimoire-forge-dialog-hotkey-capture` (`dev/done-030`) as a commit-time inline check inside the dialog. The plugin-load `Notice` remains as a backstop for frontmatter hand-edits.
