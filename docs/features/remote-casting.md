...
> `dev/done-012` — 2026-05-14 — Wires the dispatcher and forge imprinter to actually call a portal when the **Remote execution** toggle is on, completing the seam laid down by `remote-casting-setup` and giving the agent log its first producer. *(File originally `cast-log-agent.jsonl`; renamed to `cast-log-agent.json` by `cast-log-mobile-sync` so Obsidian Sync's `.obsidian/` allow-list delivers it to mobile.)*

## What it does

With the toggle in *Local* (the default), nothing changes — every cast still spawns Claude Code locally and writes to `cast-log-plugin.json` (`.jsonl` at the time of this iteration; renamed by `cast-log-mobile-sync`). Flipping the toggle to *Remote* re-routes both live casts and forge imprints through a new HTTP transport: each dispatch POSTs to `<scheme>://<portalHost>[:<port>][<path>]/cast` with `Authorization: Basic …` and a JSON body carrying the cast id, spell path, user prompt, model, and effort. The popup closes the moment the request is dispatched, and a *"Casting '<spell>' on portal…"* (or *"Forging '<name>' on portal…"*) toast confirms the remote branch took effect. *(At this iteration, forge sent the meta-spell inline as `userPrompt` with `<forge>` as the `spellPath` sentinel; superseded by `forge-spell-materialization`, where forge sends the real `.obsidian/plugins/grimoire/forge.md` path and a small per-cast `userPrompt`.)*

Each remote dispatch writes its events to `cast-log-agent.json` (`.jsonl` at the time of this iteration; see `cast-log-mobile-sync`) instead of the local log. The Logs panel already merges both files (see `cast-log-panel`), so remote casts surface in the UI without any panel-side change. A new optional `portalCastId` field on the `casted` event captures the portal's server-side id when the response lands. Five named error shapes — empty host, network failure, 401, other non-2xx, and a 30 s timeout — each surface a specific notice; four of them write an `error` event to the remote log.
...
- `CastLogStore` accepts `{ remote: true }` to direct writes to the agent log. *(Removed by `cast-unification`; each observer now holds a `CastLogWriter` bound to one log path.)*
...
- **Gives `cast-log-foundation`'s reserved agent-log path its first producer.** The schema and reader plumbing already existed; only the writers were missing. *(File later renamed `.jsonl` → `.json` by `cast-log-mobile-sync`.)*
