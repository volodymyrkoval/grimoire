...
- **Successful cast logging:** previously wrote one `casted` line and nothing else on success. Now writes `casted` to `cast-log-plugin.json` and `in-progress` → `done` (with `affectedFiles`) to `cast-log-agent.json` (both `.jsonl` at the time of this iteration; later renamed by `cast-log-mobile-sync`). Reason: complete lifecycle visibility was the original schema's goal; file split prevents sync conflicts.
