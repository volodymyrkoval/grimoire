...
The mechanism is platform hooks, not model-honoured instructions. On every plugin load, Grimoire writes three POSIX shell scripts (`session-start.sh`, `post-tool-use.sh`, `stop.sh`) into `<vault>/.obsidian/plugins/grimoire/hooks/`. Wiring them into Claude Code is a one-time manual step: the user adds a `hooks` block to `.claude/settings.local.json` at the vault root that points at the three relative script paths (the README documents the exact JSON snippet). The scripts read `$CAST_ID` from the subprocess env (already exported by `cast-log-foundation`) and append directly to the JSONL log, independent of the plugin process.
...
- `HookMaterializer` writes three `.sh` scripts (mode `0o755`) into `<plugin-dir>/hooks/` on every `onload`.
...
- README "Cast log hooks" sub-section under "Claude Code setup" documenting the manual `.claude/settings.local.json` snippet.
- POSIX shell integration tests that run the scripts under real `/bin/sh` against a temp directory.

**Out:**

- **Auto-injection of hooks into Claude Code settings** — explored and rejected after `--settings` was found to replace rather than merge user settings (see Design decisions). Manual setup is the durable answer until Claude Code grows a merge semantic.
...
- **Adds new on-disk surfaces** under `<plugin-dir>/`: `hooks/` (scripts) and `cast-log-scratch/` (transient per-cast files). Both are gitignored.
- **Adds a documented manual setup step** in the README's "Claude Code setup" section — symmetric with the existing `.claude/settings.local.json` permissions block users already maintain.

## Behavior changes

- **Successful cast logging:** previously wrote one `casted` line and nothing else on success. Now writes three lines (`casted` → `in-progress` → `done` with `affectedFiles`), provided the user has wired the hooks. Reason: complete lifecycle visibility was the original schema's goal.
...
- **Plugin `onload` side effects:** previously only constructed in-memory state. Now also writes three script files into `<plugin-dir>/hooks/` and sweeps stale scratch files. Failures are logged and the plugin still loads; hooks are observability, not load-bearing.
- **Required user setup:** previously none beyond installing the plugin. Now the user must paste a `hooks` block into `.claude/settings.local.json` to receive `in-progress` / `done` events. Reason: Claude Code's `--settings` flag replaces rather than merges, so plugin-side injection would clobber the user's own config.
