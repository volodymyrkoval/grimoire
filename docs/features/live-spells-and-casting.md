...
- **Resolves the spell's casting block** — since `grimoire-spell-local-casting-settings` (`dev/done-034`), the Enter-from-list path reads the spell's `grimoire-casting` frontmatter and resolves through `resolveCastingForSpell`, falling back to `defaults.defaultModel` / `defaults.defaultEffort` value-by-value (or wholesale when the block is absent or stale). Previously this path always used settings defaults and per-spell overrides applied only when the options panel was opened (see `options-panel`).
