...
Every cast dispatched by the plugin — Spell Picker live cast or Forge imprint — now produces an auditable record in `<vault>/<plugin-dir>/cast-log-plugin.json` (originally `cast-log-plugin.jsonl`; renamed by `cast-log-mobile-sync` so Obsidian Sync delivers it to mobile — contents are still newline-delimited JSON). The file is append-only, one JSON event per line, created on first write.
...
- **`FORGE_SPELL_PATH = '<forge>'` and `REFINE_SPELL_PATH = '<refine>'`** identify casts originating from the corresponding trigger (Forge form vs. Refine sentinel). Originally exported from the cast-log types module; relocated to `domain/spells/SystemSpellPaths.ts` in `audit-002-rework` to close cross-module boundary violations (forge / refine / castLog all consume the constants). *(Since `forge-spell-update`, `FORGE_UPDATE_SPELL_PATH = '<forge:update>'` joins the same module to tag spell-update casts; display-name resolution for all three now consults a `SystemSpellRegistry` instead of inline branching.)*
...
- `cast-log-agent.json` — produced by hook scripts for `in-progress` / `done` events (both local and portal casts). First remote-cast `casted` writes landed in `remote-casting` (`dev/done-012`). *(Originally `cast-log-agent.jsonl`; renamed by `cast-log-mobile-sync`.)*
...
- **`CastDispatcher` / `ForgeImprinter` / `CastRunner` constructor surfaces:** dispatcher and imprinter deps now require `castLogStore`; `CastRunInput` now requires `castId`. Reason: required (not optional) prevents silent log gaps. *(After `cast-unification` (`dev/done-013`) the dep is named `logWriter: CastLogWriter` rather than `castLogStore`, and the store no longer carries a routing flag — but the "required, not optional" contract is unchanged.)*
