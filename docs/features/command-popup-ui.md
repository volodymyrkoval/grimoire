...
| `CastLogPanel` | `logs` | Live folded view of `cast-log-plugin.json` + `cast-log-agent.json` (see `cast-log-panel`; both files were `.jsonl` until `cast-log-mobile-sync` renamed them) | `confirm`/`move` are no-ops — keyboard navigation is intentionally absent | Owns refresh + tick coordinators; popup calls `panel.unmount()` from `onClose` |
