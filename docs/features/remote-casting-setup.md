...
- **Forward-references** the existing agent-log reserved path in `cast-log-foundation` (`cast-log-agent.jsonl` at the time of this iteration; later renamed to `cast-log-agent.json` by `cast-log-mobile-sync`) — nothing yet writes that file; this iteration brings dispatch one step closer.
