...
- **Shared `appendRowHint` helper.** Both `SpellRow` and the Refine `SentinelRow` draw the chip from one function so chip vocabulary cannot drift between the two row types. Forge keeps no chip via a `showHint: false` default. (Since `clickable-options-chip`, the helper takes an optional `onOptionsClick` callback and emits the chip as a wrapper `<span class="spells-row-hint">` containing a cast-hint span and an interactive `.grimoire-options-chip` span.)
