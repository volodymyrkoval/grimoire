...
```
                ...
                │   - reads current spell content (Vault.read)       │
                │   - extracts @cast directives if checkbox checked  │
...
|---|---|---|---|
| `ForgeMode` discriminated union | `src/forge/ForgeMode.ts` (new) | `{ kind: 'create' } \| { kind: 'update'; spell: Spell; currentContent: string; directiveCount: number }` — carries everything the dialog needs to render itself in either mode. Honest LSP-safe substitution. | NEW |
...
| `buildForgeUpdateUserPrompt.ts` | `src/forge/buildForgeUpdateUserPrompt.ts` (new) | Pure fn: builds the small per-cast user prompt for update mode. Inputs: `{ spellPath, currentContent, description, directives: string[] \| null, model, effort }`. Parallel to existing `buildForgeUserPrompt`. | NEW |
...
| `ForgeUpdateFormSnapshot` | `src/forge/ForgeUpdateFormSnapshot.ts` (new) | Type for `{ spellPath, currentContent, description, model, effort, applyCastDirectives, directiveCount }`. Discriminant-free; the existence of `spellPath` distinguishes it from `ForgeFormSnapshot` for callers that need to. | NEW |
...
| `ForgeUpdateImprinter` | `src/forge/ForgeUpdateImprinter.ts` (new) | Update-mode pipeline: validates remote precondition, builds the per-cast user prompt, logs the cast under `<forge:update>` sentinel, dispatches via the existing `Caster` with `forge-update.md` as the system-prompt file. **Does not** rewrite the file itself — the meta-spell does that via MCP / filesystem fallback, same as create mode. | NEW |
| `castDirectiveExtractor.ts` | `src/forge/castDirectiveExtractor.ts` (new) | Pure fn `extractCastDirectives(body: string): string[]` — returns the full text of each `@cast …` line. Pure fn `countCastDirectives(body: string): number`. Both use `CAST_LINE_REGEX` from `src/editor/castLineRegex.ts` — **single source of truth** for the regex (no copy-paste). | NEW |
| `SpellContentReader` port | `src/forge/SpellContentReader.ts` (new) | Tiny interface `{ read(path: SpellPath): Promise<string> }`. Default impl wraps `app.vault.cachedRead(file)`. **DIP seam** — keeps `ForgeUpdateImprinter` import-free of `obsidian`. | NEW |
...
| `CastLogModule` | `src/main/CastLogModule.ts` (modify) | Materialize forge-update file alongside forge file. One extra `ForgeUpdateMaterializer` constructed in `initStartupMaintenance` and re-run in `materializeForge` (rename → `materializeForgeFiles()` if a second file lands the method name plural — see Q5). | MODIFIED |
...
export type ForgeMode =
  ...
  | {
      kind: 'update';
      spell: Spell;
      currentContent: string;     // full file body (incl. frontmatter)
      directiveCount: number;     // 0 → Apply @cast checkbox hidden
    };
...
export interface ForgeUpdateFormSnapshot {
  ...
  readonly directiveCount: number;     // for the notice and the user-prompt
}
...
export interface ForgeUpdateUserPromptInput {
  ...
  readonly currentContent: string;
  readonly description: string;
  readonly directives: readonly string[] | null;  // null → checkbox unchecked / no directives
...
User clicks Forge button on OptionsPanel for spell X
  ...
  → spellContentReader.read(spell.path)         // async; await
  → directiveCount = countCastDirectives(content)
  → mode = { kind: 'update', spell, currentContent: content, directiveCount }
...
ForgeSentinelDetail submit (update mode)
  → callbacks.onUpdateSubmit({
      spellPath, spellName, currentContent, description,
  ...
  → updateImprinter.imprint(snapshot, settings, close)
      ...
      ├── castId = generateId()
      ├── directives = snapshot.applyCastDirectives
      │     ? extractCastDirectives(snapshot.currentContent)
      │     : null
...
| Spell deleted between dialog open and submit | Read-once snapshot in the dialog: the dialog already holds `currentContent` from open-time. The cast still sends the captured content; the meta-spell will MCP-create the file (matching the spell's name) and write to the original path. This is acceptable: the user authored intent against the snapshot. Cast log shows the operation; if the path no longer exists, Obsidian materialises it on write. *(Edge case decision recorded in Q6.)* |
| Spell modified between dialog open and submit | Same read-once snapshot path; the user's update operates on the captured content. The meta-spell's write-back overwrites whatever is on disk now. Documented as accepted divergence. *(Edge case decision recorded in Q6.)* |
...
- **D1 — Cast Log sentinel rendering**. If the Cast Log row renderer contains an `if path === '<forge>' ... else if path === '<refine>'` switch (or a `switch` statement), it violates OCP — adding `<forge:update>` would make it a three-arm switch. The plan extracts that switch into `SystemSpellRegistry` (Section F). If grep finds no switch (the existing code may already use a Map), this todo collapses to "verify and add the third registration" with no refactor. **Audit step (F0) decides which case applies; either way, the result is a registry, not a switch.**
...
- **D3 — Spell-content read at update-dialog open is the new boundary that introduces an `obsidian` dependency in the router.** Rather than thread `App` deeper, this iteration extracts `SpellContentReader` as a small port (DIP) and confines `obsidian` to the composition root (`main.ts` → `CommandPopupBuilder`). The router's existing `app: App` field is retained for ContextNotesInput's vault search — it's already a leak, but tightening that is out of scope here. **In-scope: do not propagate that leak to a new code path.**
...
- **K2. Spell content is read once at dialog-open time and captured in the `ForgeMode` value.** No re-read at submit. *Rationale:* (a) the user's description is written against what they saw; (b) re-reading at submit invites a TOCTOU surprise where the on-disk file changed and the description no longer matches; (c) matches Refine's snapshot-on-cast model. (See Open question Q6.)
...
- **Where to read the spell body.** Use `app.vault.cachedRead(file)` (not `Vault.read`). `cachedRead` is documented for when you do not intend to mutate the file shortly after — exactly our case. The meta-spell will write later via MCP / FS; that write is independent of our read.
...
- **`buildForgeUpdateUserPrompt` shape.** A small structured block (parallel to `buildForgeUserPrompt`):
  ```
  ...
  > <description>
  - **Current spell content:**
  ```<lang or empty>
  <currentContent>
  ```
  - **Extracted @cast directives (in document order):**
  - <directive 1>
  - <directive 2>
  …
  ```
  Pure function; tested with substring assertions for each section header + edge cases (`directives === null` omits the section entirely; `directives === []` keeps the header with an explicit "(none extracted)").
...
- **Materializer lifecycle.** Mirror `018`'s `ForgeMaterializer` exactly — `CastLogModule.initStartupMaintenance` awaits both materializers; settings save fires both via `materializeForgeFiles()` (rename of `materializeForge()` — see Q5).
...
These are pitch ambiguities that the user should resolve before implementation, or that I'm flagging as my best-judgement defaults so the user can override:

- **Q1. `@cast` directive regex.** Confirmed source: `src/editor/castLineRegex.ts` exports `CAST_LINE_REGEX = /^@cast(?=\s|$)/`. The plan uses this single source. *Default:* use the existing regex verbatim. **Decision needed:** confirm we don't want a stricter pattern (e.g. require the directive to be on its own line, not just line-start).
- **Q2. Spell-file read timing.** Read once at dialog open (K2) or re-read at submit? *Plan default:* read once. **Decision needed:** confirm.
- **Q3. Behaviour when `@cast` directives are present but checkbox is *unchecked*.** Three possible semantics:
  1. **Preserve** the directives in the file (current draft default). The meta-spell sees `applyCastDirectives: false`, ignores them, and the lines remain in the rewritten body.
  2. **Strip** the directives anyway (treats them as noise the user explicitly dismissed).
  3. **Refuse to submit** when directives present and unchecked (forces a decision).
  *Plan default:* option 1 (preserve). The meta-spell's user prompt names `applyCastDirectives: false` and lists no directives; the system prompt instructs Claude to only strip when explicitly applied. **Decision needed.**
- **Q4. Cast Log sentinel switch — does it exist today?** The plan's Section F has an audit step (F0). If no switch exists (already a lookup), Section F shrinks. **Decision needed only if the implementer wants to skip the audit** — the audit is cheap.
- **Q5. `CastLogModule.materializeForge()` rename.** The method currently materialises one file; adding a second makes the singular name misleading. *Plan default:* rename to `materializeForgeFiles()`. **Decision needed:** confirm or override with another name (`runForgeMaterializers()`?).
- **Q6. Spell modified between dialog open and submit.** *Plan default:* accept divergence; the cast operates on the captured content (K2). **Decision needed:** confirm we don't want a "spell changed since you opened the dialog — re-read?" notice.
- **Q7. Submit-button disable rule.** *Plan default:* disable when description is empty AND (`applyCastDirectives` is false OR `directiveCount === 0`). **Decision needed:** confirm or specify a different rule (e.g. always allow submit; let the meta-spell handle empty inputs).
- **Q8. Active note vs closed note.** The pitch mentions "the user opens the spell note, leaves `@cast` lines inline" — does the spell need to be open in the editor to update? *Plan default:* no, the spell can be closed; the Forge button reads via `app.vault.cachedRead` regardless of editor state. **Decision needed:** confirm.
- **Q9. Spell's saved model/effort defaults vs dialog's chosen values for the update cast.** Today, opening the options panel applies `SpellOverrideStore` defaults. The user can change model/effort in the panel; clicking *Set as default* persists them. When they then click *Forge*, the dialog opens with: (a) the model/effort the panel showed (panel's live values), (b) the spell's stored override (if any), or (c) the global settings default. *Plan default:* (a) — pass the panel's live `OptionsFormState.snapshot()` model+effort to the dialog as its `FormDefaults`. **Decision needed:** confirm. *(Alternative: always start from the spell's stored override / global default, treating Forge update as independent of the panel's transient state.)*
- **Q10. Static name field UX.** Render as a disabled `<input>` (looks like the create-mode name field but greyed) or as a `<div>` with caption styling (clearly not editable)? *Plan default:* `<div>` with a small `<label>Updating spell:</label>` above and the name in a non-form element below. **Decision needed.**
...
- [ ] A1: add `export const FORGE_UPDATE_SPELL_PATH = '<forge:update>' as const;` to `src/domain/spells/SystemSpellPaths.ts`, immediately below `REFINE_SPELL_PATH`. — S, junior-dev
...
- [ ] A3: create `src/forge/ForgeMode.ts` exporting the `ForgeMode` discriminated union per Interfaces. Import `Spell` from `src/domain/spells/Spell.ts`. — S, junior-dev
- [ ] A4: create `src/forge/ForgeUpdateFormSnapshot.ts` exporting the `ForgeUpdateFormSnapshot` interface per Interfaces. Import `Effort` and `ModelId` and `SpellPath` from their existing homes. — S, junior-dev
- [ ] A5: create `src/forge/SpellImprinter.ts` exporting the `SpellImprinter<S>` generic interface per Interfaces. JSDoc: "DIP seam for create/update Forge pipelines. Either implementation must honestly satisfy the `imprint` contract — no `NotImplemented` throws, no callers needing `instanceof`." — S, junior-dev
- [ ] A6: add `implements SpellImprinter<ForgeFormSnapshot>` to the class declaration of `ForgeImprinter` in `src/forge/ForgeImprinter.ts`. **Do not change any method body or import.** `tests/ForgeImprinter.test.ts` must pass unchanged (LSP gate). — S, junior-dev
- [ ] A7: confirm `npm test`, `npm run lint`, `npm run arch:check` all green. — S, junior-dev
...
- [ ] B1: create `src/forge/forgeUpdateTemplate.ts` exporting `ForgeUpdateSystemPromptInput` and `renderForgeUpdateSystemPrompt(input)`. Body must contain: (a) `Auto-generated by Grimoire ForgeUpdateMaterializer. Do not edit — overwritten on every plugin load and settings save.` banner in `%%` block; (b) Execution Mode callout (IMMEDIATE EXECUTION); (c) MCP Tools section with `vaultMountPath` substituted; (d) **explicit preserve-frontmatter instruction** (verbatim K8 wording); (e) **explicit `@cast` directive-removal instruction** matching Refine Mode 3 step 5 wording; (f) **prefer line-level patches** rule (K9). Must NOT contain `description`, `name`, `model`, `effort`, `applyCastDirectives`, `currentContent`, or `directiveCount` (those live in the user prompt). — M, junior-dev
- [ ] B2: create `tests/forge/forgeUpdateTemplate.test.ts` with one `toContain` per invariant in B1's spec (Execution Mode, MCP Tools, `vaultMountPath` substitution, preserve-frontmatter sentence, `@cast` removal sentence, line-level-patches sentence, banner present, no leakage of per-cast names). Mirror the shape of `tests/forge/forgeTemplate.test.ts`. — S, junior-dev
- [ ] B3: create `src/forge/buildForgeUpdateUserPrompt.ts` exporting `ForgeUpdateUserPromptInput` and `buildForgeUpdateUserPrompt(input)` per Interfaces. Use the structured-block shape from Technical notes. Branches: `directives === null` omits the directives section header; `directives === []` keeps the header with `(none extracted)`; `directives === [...]` lists each as a `- ` bullet in document order. `effort === null` renders as `n/a`. — S, junior-dev
- [ ] B4: create `tests/forge/buildForgeUpdateUserPrompt.test.ts` covering: spellPath/spellName/model/effort/description appear; `effort: medium` and `effort: n/a` branches; `applyCastDirectives: true` / `false` branches; `directives === null` (no header); `directives === []` (`(none extracted)`); `directives === ['@cast tighten', '@cast add example']` (both bullets, in order); `currentContent` appears in a fenced code block. — S, junior-dev
- [ ] B5: create `src/forge/castDirectiveExtractor.ts` exporting `countCastDirectives(body: string): number` and `extractCastDirectives(body: string): string[]`. Both import `CAST_LINE_REGEX` from `src/editor/castLineRegex.ts` — **no duplicate regex literal**. Implementation: `body.split('\n').filter(line => CAST_LINE_REGEX.test(line))` → `.length` for count, the array itself for extract. — S, junior-dev
- [ ] B6: create `tests/forge/castDirectiveExtractor.test.ts` covering: empty body → 0 / `[]`; body with no `@cast` lines → 0 / `[]`; one `@cast tighten the structure` line → 1 / `['@cast tighten the structure']`; multiple directives in document order; `@casting` and `@castaway` and indented `  @cast foo` correctly excluded (regex `^@cast(?=\s|$)`); `@cast` alone on a line (no args) → 1 / `['@cast']`. — S, junior-dev
- [ ] B7: create `src/forge/SpellContentReader.ts` exporting the `SpellContentReader` interface per Interfaces. One-method interface; JSDoc names the DIP intent. No production adapter in this section — that lives in G. — S, junior-dev
- [ ] B8: confirm `npm test`, `npm run lint`, `npm run arch:check` all green. — S, junior-dev
...
- Update-mode submit builds the snapshot from: `mode.spell.path` / `mode.spell.name`, `mode.currentContent`, the description textarea, model select, effort row, the checkbox value, and `mode.directiveCount`.
...
6. The captured snapshot in update mode carries `spellPath`, `spellName`, `currentContent`, and `directiveCount` verbatim from `mode`.
...
- [ ] C0: write `tests/integration/forge-sentinel-detail-update.spec.ts` covering the six assertions above. Use `vi.fn()` for `onBack`, `onCreateSubmit`, `onUpdateSubmit`. Construct `mode` as a plain object (create or update variant). Mount the dialog with `new ForgeSentinelDetail(new Scope())` and call `render(...)` against a `document.createElement('div')`. — M, ui-integration-tester

**senior-dev**

- [ ] C1: modify `src/ui/components/ForgeSentinelDetail.ts` to accept `mode: ForgeMode` in `ForgeSentinelDetailParams`. Change `callbacks` to `{ onBack; onCreateSubmit: (s: ForgeFormSnapshot) => void; onUpdateSubmit: (s: ForgeUpdateFormSnapshot) => void }`. Add private mode storage. — M, senior-dev
- [ ] C2: in `ForgeSentinelDetail.render`, branch the name-field renderer on `mode.kind`: create → existing `#buildNameField` returning `<input>`; update → new `#buildStaticNameField(mode.spell.name)` returning a `<div>`. Branch the description placeholder on `mode.kind`. The submit handler reads the right snapshot shape and calls the right callback. — M, senior-dev
- [ ] C3: in `ForgeSentinelDetail.render`, replace the unconditional `#buildExecuteOnNoteCheckbox` call with a mode-aware version: create → existing checkbox; update + `directiveCount > 0` → new `#buildApplyCastDirectivesCheckbox(directiveCount)`; update + `directiveCount === 0` → no checkbox. Implement the singular/plural label per R14. Internal state field `#applyCastDirectives: boolean` defaults `true`. — M, senior-dev
- [ ] C4: implement the submit-button disable rule per Q7 default. Track in a private `#updateSubmitButtonState(description, applyCastDirectives, directiveCount)` method; call from the description textarea's `input` listener and the checkbox's `change` listener; on initial render, call once with the starting values. In create mode the rule degrades to "always enabled" (omit the bindings or have the method no-op when `mode.kind === 'create'`). — M, senior-dev
- [ ] C5: update the existing `tests/integration/forge-sentinel-detail.spec.ts` to use the new `callbacks` shape (`onCreateSubmit` in place of `onSubmit`). Existing assertions (renders the name input, description textarea, model select, execute-on-note checkbox; submits with a `ForgeFormSnapshot`) remain — they just reference the new callback name. — S, senior-dev
- [ ] C6: confirm C0 GREEN, existing `tests/integration/forge-sentinel-detail.spec.ts` GREEN after the rename, `npm test`, `npm run lint`, `npm run arch:check`, `npm run test:integration` all green. — S, senior-dev
...
- `renderForgeUpdate` reads spell content via `await this.#deps.spellContentReader.read(spell.path)`. On read failure: `new Notice('Could not read spell content'); this.#deps.onExit();` and return (no detail-phase enter). On success: build `ForgeMode.update`, mount `ForgeSentinelDetail`, pass live model/effort defaults (Q9 default — pulled from settings; if the panel-snapshot threading proves complex, fall back to global `formDefaults` and the user re-picks model/effort in the dialog; **document the choice in the implementation**).
...
- [ ] D0: write `tests/integration/forge-update-popup.spec.ts` covering the five assertions above. Use the existing popup test harness; extend it to forward `forgeUpdateAction?` and `spellContentReader?` (default: `vi.fn()` and a stub returning a body with two `@cast` lines for the directive-count assertion). — M, ui-integration-tester

**senior-dev**

- [ ] D1: modify `src/ui/options/OptionsPanel.ts` per K7: add `onForgeUpdate?: () => void` to `OptionsPanelDeps`; in `#buildFormControls`, after the existing `buttonRow`, conditionally create a `grimoire-forge-update-row` div and a `'Forge'` button (`type="button"`) whose click handler calls `deps.onForgeUpdate?.()`. — S, senior-dev
- [ ] D2: modify `src/ui/components/OptionsDetail.ts`: add `onForgeUpdate?: (spell: Spell) => void` to `OptionsDetailParams`. In `#createPanel`, when `params.kind.kind === 'spell' && params.onForgeUpdate`, pass `onForgeUpdate: () => params.onForgeUpdate!(params.kind.spell)` to `OptionsPanel.render` deps. Otherwise omit. — S, senior-dev
- [ ] D3: modify `src/ui/popup/DetailPanelRouter.ts`: add `forgeUpdateAction: ForgeUpdateAction` and `spellContentReader: SpellContentReader` to `DetailPanelRouterDeps`. Export `ForgeUpdateAction = (spell: Spell) => void` from the same module (parallel to existing action types). In `renderSpellOptions`, pass `onForgeUpdate: (spell) => { exit(); void this.renderForgeUpdate(contentEl, scope, spell); }` to OptionsDetail params. — S, senior-dev
- [ ] D4: implement `DetailPanelRouter.renderForgeUpdate(contentEl, scope, spell)` per Design context: `reattachTabBar`, async-read content, on success build `ForgeMode.update`, mount `ForgeSentinelDetail` with the new callback shape, wire `onUpdateSubmit: (snapshot) => { this.#deps.forgeUpdateAction(spell); /* snapshot flows via the dialog's wiring; see E */ exit(); }`. Refer to the E section for how the snapshot reaches the imprinter — D's job is to *capture and forward* the snapshot via a per-dialog closure that the imprinter receives. **Implementation detail:** the cleanest wiring is `forgeUpdateAction = (spell, snapshot) => imprinter.imprint(snapshot, settings, close)` — i.e. extend `ForgeUpdateAction = (spell: Spell, snapshot: ForgeUpdateFormSnapshot) => void`. Adjust the type accordingly. — M, senior-dev
- [ ] D5: update `DetailPanelRouter.renderForge` for the new `ForgeSentinelDetail` callback shape (`onCreateSubmit` instead of `onSubmit`). Pass `mode: { kind: 'create' }` and `callbacks: { onBack: exit, onCreateSubmit: (snap) => { this.#deps.imprintAction(snap); exit(); }, onUpdateSubmit: () => { /* never called in create mode */ } }`. — S, senior-dev
- [ ] D6: modify `src/ui/CommandPopup.ts`: add `forgeUpdateAction: ForgeUpdateAction` and `spellContentReader: SpellContentReader` to `CommandPopupParams`. Store on private fields. Thread into `DetailPanelRouter` deps in `#buildRouter`. Export `ForgeUpdateAction` re-export from `CommandPopup.ts` per the existing convention. — S, senior-dev
- [ ] D7: update `tests/CommandPopup.test.ts` and `tests/integration/harness.ts` (or equivalent) to pass `forgeUpdateAction: vi.fn()` and `spellContentReader: { read: vi.fn(async () => '') }` in all `CommandPopup` constructions. — S, senior-dev
- [ ] D8: confirm D0 GREEN, existing options-panel tests GREEN, `npm test`, `npm run lint`, `npm run arch:check`, `npm run test:integration` all green. — S, senior-dev
...
- [ ] E1: create `src/forge/ForgeUpdateMaterializer.ts` mirroring `src/forge/ForgeMaterializer.ts` shape. Same ports interface (renamed: `getForgeUpdatePathAbs`, `getSettings: () => ForgeUpdateSystemPromptInput`). Same guard. Same `run()` body, calling `renderForgeUpdateSystemPrompt` from B1. — S, junior-dev
- [ ] E2: create `tests/forge/ForgeUpdateMaterializer.test.ts` mirroring `tests/forge/ForgeMaterializer.test.ts`. Cover: mkdir called once with the plugin dir; writeFile called once with the forge-update file path; content matches `renderForgeUpdateSystemPrompt` for given settings; rejection on writeFile propagates; default ports construct without error. — S, junior-dev

**senior-dev**

- [ ] E3: create `src/forge/ForgeUpdateImprinter.ts` implementing `SpellImprinter<ForgeUpdateFormSnapshot>`. Body parallels `ForgeImprinter.imprint` per Design context. Use `FORGE_UPDATE_SPELL_PATH` for the log-write sentinel. Use `buildForgeUpdateUserPrompt`. Use `extractCastDirectives` (B5) when `snapshot.applyCastDirectives === true`. Notice strings as specified in Design context. — M, senior-dev
- [ ] E4: create `tests/forge/ForgeUpdateImprinter.test.ts` covering: remote-without-portal-host → notify + return + no cast; happy path local → notify with "Updating" → close → logWriter.recordCasted with `FORGE_UPDATE_SPELL_PATH` → cast called with `systemPromptFile` + `spellPath`; happy path remote → notify with "on portal…" + onAccepted re-records with `portalCastId`; failure path → logWriter.recordError + notify with "Forge update failed:" prefix; `applyCastDirectives: true` → `extractCastDirectives` called with `currentContent`, result passed into `buildForgeUpdateUserPrompt`; `applyCastDirectives: false` → directives passed as `null`. Stub the `Caster`, `CastEventSink`, and `notify`. Stub `forgeUpdateSpellPaths` to return literal paths. — M, senior-dev
- [ ] E5: confirm `npm test`, `npm run lint`, `npm run arch:check` all green. — S, senior-dev
...
- [ ] F0: audit step. `grep -rn "'<forge>'\|FORGE_SPELL_PATH\|'<refine>'\|REFINE_SPELL_PATH" src/castLog src/ui` to locate every site that compares against a system sentinel. Determine which of the three outcomes applies (switch / lookup-table / no centralisation). Document the finding inline in the F1 commit message. — S, senior-dev
- [ ] F1: create `src/castLog/SystemSpellRegistry.ts` per Interfaces (`register`, `describe`, `isSystemSpell`). Stateless API — the instance holds the map. Pre-register nothing in the class itself; registrations happen at construction time in `main.ts`. — S, senior-dev
- [ ] F2: based on F0's finding, refactor the call site(s) to consult `SystemSpellRegistry`. If a switch exists → replace with `registry.describe(path)?.label ?? path`. If a lookup table exists → migrate its content into the registry. If neither → identify the most natural single seam in the Cast Log row renderer and route through the registry. **Keep the change minimal** — this is a structural refactor, not a feature change. — M, senior-dev
- [ ] F3: create `tests/castLog/SystemSpellRegistry.test.ts` covering: empty registry → `describe` returns undefined for any path; `register` then `describe` round-trip; `isSystemSpell` returns true after registration; `register` with duplicate path overwrites (or throws — implementer's call, document); multiple registrations independent. — S, senior-dev
- [ ] F4: if F0 found existing tests on the Cast Log row renderer, extend them with a `<forge:update>` → `"Forge (update)"` case. If no such tests exist, **do not** invent a new test file in F — the integration test in G's verification will cover this end-to-end. — S, senior-dev
- [ ] F5: confirm `npm test`, `npm run lint`, `npm run arch:check` all green. — S, senior-dev
...
- [ ] G0: write `tests/integration/forge-update-end-to-end.spec.ts` per the Red criterion above. Use a real `ForgeUpdateImprinter` with a stubbed `Caster` (assert `cast` calls), a stubbed `CastEventSink` (assert `recordCasted` / `recordError`), and a stubbed `SpellContentReader` returning a spell body with at least one `@cast` line (so the directive-count assertion is non-trivial). — M, ui-integration-tester

**senior-dev**

- [ ] G1: modify `src/main/CastLogModule.ts`: in `initStartupMaintenance`, construct and `await run()` on a `ForgeUpdateMaterializer` alongside the existing `ForgeMaterializer`. Rename `materializeForge` method to `materializeForgeFiles` and have it fire both materializers (fire-and-forget pattern preserved). — M, senior-dev
- [ ] G2: update `src/main.ts` per Design context: build the `ForgeUpdateSystemPromptInput` thunk (only `vaultMountPath`), pass the new `forgeUpdateSpellPaths` thunk to `PopupModule` (mirror the existing `forgeSpellPaths` thunk wiring), construct the `SpellContentReader` adapter, construct the `SystemSpellRegistry` instance and pre-register three sentinels, update the `GrimoireSettingTab` callback to call `materializeForgeFiles` (the renamed method). — M, senior-dev
- [ ] G3: modify `src/main/PopupModule.ts` (or wherever `CommandPopupBuilder` is constructed) to accept and forward `forgeUpdateSpellPaths` and `spellContentReader`. — S, senior-dev
- [ ] G4: modify `src/ui/popup/CommandPopupBuilder.ts` per Design context: accept `forgeUpdateSpellPaths` and `spellContentReader` in `CommandPopupBuilderDeps`; construct one `ForgeUpdateImprinter` in `build()`; produce the `forgeUpdateAction` closure (signature `(spell, snapshot) => updateImprinter.imprint(snapshot, this.#deps.plugin.data.settings, () => popup.close())`); pass `forgeUpdateAction` and `spellContentReader` into `CommandPopup` via the new params. — M, senior-dev
- [ ] G5: extend `tests/main.test.ts` with two cases: (a) onload constructs `ForgeUpdateMaterializer` and awaits its `run()`; (b) the popup builder receives a `spellContentReader` and a `forgeUpdateSpellPaths` thunk that, when invoked, returns paths derived from `vaultMountPath` and `PluginPaths.forgeUpdateSpellPathVaultRel()`. — M, senior-dev
- [ ] G6: confirm G0 GREEN, all existing tests GREEN, `npm test`, `npm run lint`, `npm run arch:check`, `npm run test:integration` all green. — S, senior-dev
...
- [ ] H1: read `docs/features/forge-cast.md` and `docs/features/forge-spell-materialization.md`. Apply minimal in-place edits to describe the update mode alongside create. Reference plan 030 by number. Do not duplicate the plan's contents — link by reference. — S, junior-dev
- [ ] H2: confirm `npm run lint`, `npm run arch:check` still pass. — S, junior-dev
